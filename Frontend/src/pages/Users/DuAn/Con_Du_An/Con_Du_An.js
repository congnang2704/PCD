...
export default function Con_Du_An() {
  ...
  // load data: FE tự sort + tự phân trang (đảm bảo luôn ra 9 dự án mới nhất)
  useEffect(() => {
    let mounted = true;
    ...
    (async () => {
      try {
        ...
        // cache FULL LIST 3 phút (nhẹ hơn, đổi trang không cần gọi API lại)
        const cacheKey = `cache_projects_full_list_v1`;
        ...
        if (isCacheValid) {
          ...
          full = sortNewestFirst((parsed?.list || []).filter((p) => !!p?.name));
        } else {
          // gọi API (dù backend có phân trang hay không, FE vẫn tự xử lý)
          const res = await getProjectsPage({
            page: 1,
            limit: 9999, // cố lấy đủ để FE tự phân trang (nếu backend có limit vẫn ok)
            timeout: 12000,
        ...
        // nếu page đang lớn hơn tp (do data ít lại), kéo về trang cuối hợp lệ
  ...
  // chỉ extract ảnh cho 9 dự án đang hiển thị
