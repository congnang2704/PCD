...
const CATEGORY_NAME = "Nhà 5 tầng";
...
const Container_HouseFive = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 1) Show cache ngay (giảm TTI cảm nhận)
      const cacheKey = `house-five:${CATEGORY_NAME}`;
      ...
      // 2) Revalidate nền – gọi API nhẹ (fields + status + sort + limit)
      try {
        const q = buildQuery({
          status: "published",
          sort: "-published_at,-created_at",
          limit: 400, // đủ rộng để lọc 1 danh mục
          fields:
  ...
  return (
    <div className="house-container">
      <h2 className="house-title">MẪU NHÀ 5 TẦNG ĐẸP, ĐẲNG CẤP</h2>
      <p className="house-subtitle">
        Giải pháp tối ưu cho mặt bằng đô thị, công năng đa tầng thông minh.
      </p>
...
export default Container_HouseFive;
